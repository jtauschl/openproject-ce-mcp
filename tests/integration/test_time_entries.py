...
async def test_create_get_update_time_entry_until(
    ...
    seconds throughout: this instance's own real duration-rounding behavior
    (verified manually against a live instance) truncates a fractional-second
    `hours` remainder to whole seconds, so a fractional end_time would not
    round-trip byte-identically -- a server property, not something
    create_time_entry_until's own arithmetic could be wrong about, so not
    re-verified here.
...
async def test_create_time_entry_succeeds_for_log_own_time_only_role(
    ...
    client.py's _time_entry_activities_from_project comment): OpenProject's
