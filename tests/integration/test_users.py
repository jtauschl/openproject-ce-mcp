...
async def test_user_lifecycle_roundtrip(client: OpenProjectClient, user_ids: list[int]) -> None:
    ...
    but never actually persisted (see the `password` fix in create_user --
    the form response never echoes it back, and an earlier version of that
    method committed the form's own payload verbatim instead of restoring
    it, so a real create silently failed with "Password can't be blank."
    despite the preview reporting ready=True)."""
