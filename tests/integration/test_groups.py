...
async def test_list_groups_reports_correct_member_count(client: OpenProjectClient, group_ids: list[int]) -> None:
    """Regression: normalize_group's member_count only recognized a
    {count,...}/{total,...} collection-object shape for _embedded.members --
    OpenProject's real list response shape is a flat array, which was never
    checked, so member_count silently stayed 0 for every group returned by
    list_groups (unlike get_group, which already tolerated the real shape)."""
    me = await client.get_current_user()
    name = f"[integration-test] {uuid.uuid4().hex[:8]}"
