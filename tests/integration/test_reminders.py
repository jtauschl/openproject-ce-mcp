...
async def test_create_reminder_rejects_hidden_note_field(
    ...
    with pytest.raises(InvalidInputError, match="OPENPROJECT_HIDE_REMINDER_FIELDS"):
        await hidden_client.reminder.create(
...
async def test_list_reminders_finds_created_reminder(
    ...
    wp = await client.work_package.create(
...
async def test_update_reminder_denied_outside_write_allowlist(
    ...
    with pytest.raises(PermissionDeniedError):
        await denied_client.reminder.update(reminder_id=created.reminder_id, note="denied", confirm=True)
...
async def test_update_reminder_changes_note(
    ...
    preview = await client.reminder.update(reminder_id=created.reminder_id, note="updated by integration test")
    assert preview.state == "preview"

    updated = await client.reminder.update(
...
async def test_delete_reminder_denied_outside_write_allowlist(
    ...
    with pytest.raises(PermissionDeniedError):
        await denied_client.reminder.delete(reminder_id=created.reminder_id, confirm=True)

    # Not denied -- clean up for real via the allowed client.
    await client.reminder.delete(reminder_id=created.reminder_id, confirm=True)
...
async def test_delete_reminder_removes_it(client: OpenProjectClient, test_project: str, wp_ids: list[int]) -> None:
    ...
    deleted = await client.reminder.delete(reminder_id=created.reminder_id, confirm=True)
