...
async def test_list_news(client: OpenProjectClient, test_project: str, news_ids: list[int]) -> None:
    ...
    create_result = await client.news.create(
    ...
    result = await client.news.list(project=test_project)
...
async def test_create_get_update_delete_news(client: OpenProjectClient, test_project: str, news_ids: list[int]) -> None:
    ...
    # Create
    result = await client.news.create(
        project=test_project,
        title=title,
    ...
    # Read
    news = await client.news.get(news_id)
    assert news.title == title
    assert news.id == news_id

    # Update
    update_result = await client.news.update(
        news_id=news_id,
    ...
    updated = await client.news.get(news_id)
    assert updated.title == f"{title} updated"

    # Delete
    delete_result = await client.news.delete(news_id=news_id, confirm=True)
...
async def test_create_update_delete_news_denied_outside_write_allowlist(
    ...
    with pytest.raises(PermissionDeniedError):
        await denied_client.news.update(news_id=existing.news_id, title="denied update", confirm=True)

    with pytest.raises(PermissionDeniedError):
        await denied_client.news.delete(news_id=existing.news_id, confirm=True)
