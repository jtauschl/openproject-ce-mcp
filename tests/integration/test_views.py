...
instance (op-17-5): creating a query via `POST /api/v3/queries` then a view
via `POST /api/v3/views/work_packages_table` with `_links.query` pointing
at it succeeds and returns a real view id. This client has no create_query
