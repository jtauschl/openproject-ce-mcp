...
async def test_list_views_paginates_beyond_a_single_page(client: OpenProjectClient) -> None:
    ...
    views rather than creating new ones -- unfiltered (no project=). Uses an
    unrestricted client (read_projects=("*",)): the pre-existing views live
    under OpenProject's own demo "Scrum project" seed data, not the
    integration test project, so the default project-scoped client fixture
    would see zero of them regardless of how many actually exist on the
    instance."""
