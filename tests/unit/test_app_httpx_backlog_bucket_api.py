...
def _backlog_bucket_payload(bucket_id: int = 1, *, with_link: bool = True, with_embedded: bool = False) -> dict:
    links: dict = {}
    embedded: dict = {}
    if with_link:
...
@pytest.mark.asyncio
async def test_list_all_requests_the_backlog_buckets_endpoint() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/api/v3/backlog_buckets"
        assert dict(request.url.params) == {"offset": "1", "pageSize": "50"}
        return httpx.Response(200, json={"_embedded": {"elements": [_backlog_bucket_payload()]}}, request=request)

    async with _client(handler) as http_client:
        api = HttpxBacklogBucketApi(HttpxTransport(http_client))
...
async def test_list_all_skips_an_element_with_a_missing_id() -> None:
    """Regression test for the has_usable_id unification (mirrors
    test_app_httpx_sprint_api.py): list_* must not raise on one malformed
    element among otherwise well-formed ones -- skip it, don't fail every
    other backlog bucket."""
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            ...
            json={
                "_embedded": {
                    "elements": [{"_type": "BacklogBucket", "name": "No id here"}, _backlog_bucket_payload(2)]
                }
            },
            request=request,
        )
...
async def test_list_all_accepts_a_numeric_string_id() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        payload = _backlog_bucket_payload(7)
...
async def test_list_for_project_requests_the_project_scoped_endpoint() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/api/v3/projects/7/backlog_buckets"
        assert dict(request.url.params) == {"offset": "1", "pageSize": "50"}
        return httpx.Response(200, json={"_embedded": {"elements": [_backlog_bucket_payload()]}}, request=request)
...
async def test_list_for_project_returns_records_and_total_at_arbitrary_offset() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"total": 5, "_embedded": {"elements": [_backlog_bucket_payload(1), _backlog_bucket_payload(2)]}},
            request=request,
        )
...
@pytest.mark.asyncio
async def test_get_requests_the_single_backlog_bucket_endpoint() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/api/v3/backlog_buckets/1"
        assert request.method == "GET"
        return httpx.Response(200, json=_backlog_bucket_payload(), request=request)
...
async def test_detail_reuses_every_summary_field_verbatim() -> None:
    ...
    zero added fields -- mirrors SprintDetail."""
...
async def test_defining_workspace_link_falls_back_to_embedded_self_link() -> None:
    """Verbatim port of the sprint adapter's equivalent test: when only
    `_embedded.definingWorkspace` is present (no top-level
    `_links.definingWorkspace`), synthesize a link from the embedded
    object's own `_links.self` (+ name as a title fallback).
    """

    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_backlog_bucket_payload(with_link=False, with_embedded=True), request=request)
...
async def test_no_defining_workspace_at_all_yields_none_link_and_payload() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_backlog_bucket_payload(with_link=False, with_embedded=False), request=request)
...
async def test_not_found_propagates_unwrapped_from_the_adapter() -> None:
    ...
    "Backlogs module" message is a Service-layer concern (see
    test_app_backlog_bucket_service.py), not the adapter's.
    """
