...
async def test_search_work_packages_resolves_exact_match_by_display_id() -> None:
    ...
    result = await client.search_work_packages(query="OPM-394")
...
async def test_search_work_packages_exact_match_respects_other_filters() -> None:
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/api/v3/work_packages":
            filters = json.loads(request.url.params["filters"])
            if any("id" in f for f in filters):
                # Server applies the project_id filter too -- work package 394
                # belongs to project 2, not the scoped project 1, so no match.
                return httpx.Response(200, json={"total": 0, "_embedded": {"elements": []}}, request=request)
            return httpx.Response(200, json={"total": 0, "_embedded": {"elements": []}}, request=request)
        raise AssertionError(f"Unexpected request: {request.method} {request.url}")
    ...
    result = await client.search_work_packages(query="OPM-394", project="demo")
...
async def test_search_work_packages_exact_match_none_for_unresolvable_query() -> None:
    ...
    result = await client.search_work_packages(query="garbage text")
...
async def test_search_work_packages_exact_match_none_under_restricted_read_scope() -> None:
    ...
    result = await client.search_work_packages(query="394")
...
async def test_hidden_relation_field_is_rejected_on_update() -> None:
    ...
    settings = Settings(
        ...
        max_results=100,
        log_level="WARNING",
        hidden_fields={"relation": ("description",)},
        enable_work_package_write=True,
    )
    client = OpenProjectClient(settings, transport=httpx.MockTransport(handler))
...
@pytest.mark.asyncio
async def test_hidden_group_field_is_rejected_on_create() -> None:
    settings = Settings(
        read_projects=("*",),
        write_projects=("*",),
        base_url="https://op.example.com",
    ...
    with pytest.raises(InvalidInputError, match="hidden by OPENPROJECT_HIDE_GROUP_FIELDS"):
        await client.create_group(name="secret-team", confirm=False)

    await client.aclose()


@pytest.mark.asyncio
async def test_hidden_group_field_is_rejected_on_update() -> None:
    settings = Settings(
        read_projects=("*",),
        write_projects=("*",),
        base_url="https://op.example.com",
    ...
    with pytest.raises(InvalidInputError, match="hidden by OPENPROJECT_HIDE_GROUP_FIELDS"):
        await client.update_group(group_id=5, name="renamed", confirm=False)
...
async def test_list_users_search_not_truncated_when_exactly_limit_matches_exist() -> None:
    ...
    client = OpenProjectClient(settings, transport=httpx.MockTransport(handler))
    result = await client.list_users(search="ali", limit=1)
...
async def test_list_sprints_not_truncated_when_exactly_limit_allowed_matches_exist() -> None:
    ...
    (limit=1) and nothing follows it -- truncated must be False."""
    settings = dataclasses.replace(make_settings(), read_projects=("demo",))

    def sprint_item(item_id: int) -> dict:
        return {
            "_type": "Sprint",
            "id": item_id,
            "name": f"Sprint {item_id}",
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/api/v3/sprints" and request.method == "GET":
            ...
            requested_offsets.append(offset)
            if offset == "1":
                return httpx.Response(200, json={"_embedded": {"elements": [sprint_item(1)]}}, request=request)
            raise AssertionError(f"Unexpected offset: {offset}")
        raise AssertionError(f"Unexpected request: {request.method} {request.url}")
