"""Transport-neutral HAL+JSON normalization.

Package-root module deliberately independent of both `client.py` and `app/`
(no imports from either, no `httpx`) -- `normalize_links` is used by
client.py's legacy facade today, and is intended to be reused by any future
app/ transport port during the ongoing layered-decomposition migration
without either side importing from the other, so a neutral shared module is
the only way to keep this one real invariant in a single place instead of
duplicated.
"""
...
def normalize_links(value: Any) -> Any:
    ...
    raising AttributeError on the following `.get`.
    """
