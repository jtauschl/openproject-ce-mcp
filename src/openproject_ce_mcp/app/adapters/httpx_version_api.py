...
`_link_title`/`_normalize_validation_errors` (+ `SUBJECT_LIMIT`/`FORMATTABLE_LIMIT`) --
duplicated rather than imported from client.py to avoid `app/` importing from
`client.py` (still used by ~50 other normalize_* methods and 6 other
`_finalize_*_write` variants there). Unify only once every domain has migrated and
client.py's copies become truly dead.
"""
...
from ..ports.version_api import VersionFormResult, VersionPage, VersionRecord
...
def normalize_version(payload: dict[str, Any], *, base_url: str) -> VersionSummary:
...
def _summary_to_detail(summary: VersionSummary) -> VersionDetail:
...
def normalize_version_detail(payload: dict[str, Any], *, base_url: str) -> VersionDetail:
    return _summary_to_detail(normalize_version(payload, base_url=base_url))
...
class HttpxVersionApi:
    ...
    def _record(self, payload: dict[str, Any]) -> VersionRecord:
        return VersionRecord(
            summary=normalize_version(payload, base_url=self._base_url),
    ...
    async def list_for_project(self, project_id: int, *, offset: int, page_size: int) -> VersionPage:
        ...
        records = [
            self._record(item) for item in payload.get("_embedded", {}).get("elements", []) if isinstance(item, dict)
        ]
    ...
    async def get(self, version_id: int) -> VersionRecord:
        return self._record(await self._transport.get_json(f"versions/{version_id}"))
