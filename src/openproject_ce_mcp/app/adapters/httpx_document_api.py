...
class HttpxDocumentApi:
    ...
    def _record(self, payload: dict[str, Any]) -> DocumentRecord:
        ...
        return DocumentRecord(
            ...
            # only `payload`/`summary` (small, per-record), not `self` -- it
            # does not keep a whole adapter/transport alive. Passing
            # `summary` through avoids re-deriving every OTHER field (id,
            # title, project, ...) a second time inside the thunk.
            to_detail=lambda: normalize_document_detail(payload, summary=summary),
