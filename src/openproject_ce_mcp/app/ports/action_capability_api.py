...
`/api/v3/workspaces/{id}` link, not merely a title) -- found missing during
this domain's own step-6.5 Codex review: the pre-migration client.py only ever
allowlist-checked `list_capabilities`' `project` parameter (by resolving it
through `ProjectRefResolver` before building the `context` filter), never each
individual returned record's own `context` link. A `capability_id`-only call
...
class ActionCapabilityApi(Protocol):
    ...
    docs during this domain's step-6.5 review (a `capability_id`-only
    lookup uses this `get`, not a collection `id` filter the docs don't
    support; see `action_capability_service.py`'s module docstring).
    """
