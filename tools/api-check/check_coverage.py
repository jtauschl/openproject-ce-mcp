...
def render(rows, tally, *, live_enabled: bool) -> str:
    ...
    if not live_enabled:
        lines.append("(live probe skipped — set OPENPROJECT_BASE_URL / "
                     "OPENPROJECT_API_TOKEN for CE availability)")
    return "\n".join(lines)
...
def main() -> int:
    ...
    parser.add_argument("--write", action="store_true",
                        help="(re)write tools/api-check/COVERAGE.md")
    args = parser.parse_args()

    if not (SOURCES / SOURCE_VERSION).exists():
        print(f"error: missing source clone {SOURCE_VERSION}. "
              "Run tools/api-check/fetch-sources.sh", file=sys.stderr)
        return 2

    live_enabled = bool(os.environ.get("OPENPROJECT_BASE_URL")
                        and os.environ.get("OPENPROJECT_API_TOKEN"))
