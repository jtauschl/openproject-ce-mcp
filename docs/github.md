...
> **Note on setup output:** `openproject-ce-mcp configure --local` writes a
> Claude-format `.mcp.json` (root key `mcpServers`) in the current directory. VS Code uses a
...
1. **Create `.vscode/mcp.json` in your workspace root.** Use the `command` and
   `env` values from the `.mcp.json` generated by `openproject-ce-mcp configure --local`
   (the values are identical; only the file location and the surrounding JSON
   structure differ).
