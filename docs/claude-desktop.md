...
3. **Add the server.** Use the `env` values from the `.mcp.json` generated by
   `openproject-ce-mcp configure --local` (the root key is the same,
   `mcpServers`). With a PyPI install the command is simply
