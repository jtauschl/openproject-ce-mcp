...
1. **Let the wizard write `.cursor/mcp.json` for you.** The easiest path is to run
   `openproject-ce-mcp configure`, answer the project-scoped gate, and select
   Cursor — it writes `.cursor/mcp.json` for you. The example below shows the
   format if you prefer to create it manually. The structure is identical to
   every other client (root key `mcpServers`). With a PyPI install the command is
   simply `openproject-ce-mcp`; source installs can use the `.venv` binary path.
   ```json
   {
     "mcpServers": {
       "openproject": {
